...
/*----------------------------------------------------------------------------------------
Função para autualizar o tabuleiro do jogo*/ //ATUALIZAR !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
export async function requestUpdate(game, nick, callback, f) {
    ...
    try {
        ...
        let isMessageReceived = false; // Flag para controlar a execução de f

        const intervalId = setInterval(() => {
            if (!isMessageReceived) {
                //f(); // Executa a função enquanto não recebe mensagem FAZER O POP UP DE GAME LOADING
            }
        }, 1000); // Executa a cada 1 segundo (ajuste conforme necessário)

        eventSource.onmessage = (event) => {
            isMessageReceived = true; // Sinaliza que uma mensagem foi recebida
            clearInterval(intervalId); // Para a execução da função f
        ...
        eventSource.onerror = () => {
            ...
            clearInterval(intervalId); // Para a execução de f no caso de erro
            eventSource.close();
        };
    } catch (error) {
